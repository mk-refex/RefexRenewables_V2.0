...
export default function RelatedLinksSection() {
  ...
  const requestedCategoryKey = useMemo(() => {
    return (
      new URLSearchParams(location.search).get("categoryKey")?.trim() ?? ""
    );
  }, [location.search]);
      ...
      .then((data) => {
        ...
        if (parsed.length) {
          ...
          const matchedByContains = requested
            ? parsed.find((cat) =>
                normalize(cat.name ?? "").includes(requested),
              )
            : null;
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections.filter(
      (sec) =>
        sec.labelType === "financialYear" && sec.financialYear === filterYear,
    );
  ...
  useEffect(() => {
    ...
    const filtered =
      filterYear === "all"
        ...
        : sections.filter(
            (sec) =>
              sec.labelType === "financialYear" &&
              sec.financialYear === filterYear,
          );
              ...
              {!selectedCategory ? (
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-start gap-3">
                                        <div className="w-8 h-8 flex-shrink-0">
                                          <img
                                            src="/wp-content/uploads/2025/09/invest-file.svg"
                                            alt="file"
                                    ...
                                    {item.pdfUrl ? (
                                      <div className="flex shrink-0 flex-row gap-6 sm:gap-4">
                                        <div className="flex flex-col items-center">
                                          ...
                                          >
                                            <img
                                              src="/wp-content/uploads/2025/09/visible.svg"
                                              alt="view"
                                              className="w-4 h-4"
                                            />
                                          </a>
                                        ...
                                        <div className="flex flex-col items-center">
                                          ...
                                          >
                                            <img
                                              src="/wp-content/uploads/2025/09/investor-download.svg"
                                              alt="download"
                                              className="w-4 h-4"
                                            />
                                          </a>
                ...
                filteredSections.map((section) => {
                  ...
                  const showSectionHeading =
                    hasSectionName && !allSectionsAreFinancialYear;
                  return (
