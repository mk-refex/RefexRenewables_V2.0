...
function memberToForm(member: KeyManagerialMember): FormState {
  return {
    id: member.id,
    name: member.name,
...
function formToMember(form: FormState, order: number, fallbackId: number): KeyManagerialMember {
  return {
...
export default function KeyManagerialCMS({ showNotification }: Props) {
  ...
  const [members, setMembers] = useState<KeyManagerialMember[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      showNotification(error?.message || "Failed to load key managerial personnel", "error");
    } finally {
      setLoading(false);
    }
  }, [showNotification]);
  ...
  const openEdit = (member: KeyManagerialMember) => {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const saved = await keyManagerialApi.save({
        title: title.trim(),
      ...
      showNotification("Key managerial personnel saved successfully!", "success");
    } catch (error: any) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16 text-gray-500">
        Loading key managerial personnel…
      </div>
    );
  }
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Key Managerial Personnel"
          />
        </div>
        <div>
